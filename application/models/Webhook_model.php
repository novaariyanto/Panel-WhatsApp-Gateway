...
class Webhook_model extends CI_Model
{
    ...
    public function add($instance_key,$url, $status)
    {
    ...
    public function getAll($limit,$start)
    {
        ...
        // $query = $this->db->get_where($this->_table, ['id_user' => $user_id]);
        $this->db->select("tb_webhook.*,tb_device.device_name");
		$this->db->from($this->_table);
        $this->db->join("tb_device","tb_webhook.instance_key = tb_device.api_key");
		$this->db->where("tb_device.id_user",$user_id);
		$this->db->order_by("tb_device.id","desc");
